...
describe("Order repository test", () => {
  ...
  it("should update an order", async () => {
    ...
    let orderUpdated = await OrderModel.findOne({
      where: { id: order.id },
      include: ["items"],
    });

    expect(orderUpdated.toJSON() === order);
  });
  ...
  it("should find an order", async () => {
    ...
    expect(orderModel.toJSON() === orderResult);
  });
  ...
  it("should find all orders", async () => {
    ...
    const ordersOrderModel = orderModel.map((order) => {
      return order.toJSON();
    });

    const ordersResult = await orderRepository.findAll();

    expect(ordersOrderModel === ordersResult);
  });
